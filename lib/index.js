...
const migrateCMD = function (model) {
    _checkEnv();
    APP_PATH = getProjectAppPath(projectRootPath);
    let models = loader(APP_PATH, {
        root: 'model',
        prefix: '',
    });
    let configs = require(`${APP_PATH}/config/db.js`);
    let dbConf = configs.master || configs.database;
    if (!dbConf || !dbConf.db_type) {
        console.log();
        console.log('database config error! ');
        process.exit();
    }
    if (commander.file) {
        let sqlStr = fs.readFileSync(path.resolve(commander.file));
        let sqlArr = sqlStr.toString().split('\n'), ps = [];
        let keys = Object.keys(keys);
        model = keys[0];
        if (!model) {
            console.log();
            console.log('connot find the model! ');
            process.exit();
        }
        sqlArr.map(item => {
            ps.push(new model(dbConf).migrate(item));
        });
        ...
        if (!model) {
            console.log();
            console.log('please enter the name of the model! ');
            process.exit();
        }
        if (!models[model]) {
            console.log();
        ...
        let m = new (models[model])(dbConf);
        return m.migrate().catch(err => {
            console.log();
            console.log(err.stack);
            process.exit();
        });
...
commander.option('-f, --file', 'migrate database struct from files, used in `think migrate` command');
//create project
...
commander.command('migrate <modelName>').description('migrate model structure to database').action(function (model) {
    migrateCMD(model);
});
