...
const createView = function (template) {
    ...
    let viewPath = APP_PATH + path.sep + 'view', filename = module + path.sep + template + '.html';
    if (middlewareConfig.config && middlewareConfig.config.view) {
        ...
        if (middlewareConfig.config.view.file_depr === '_') {
            filename = module + '_' + template;
        } else {
            filename = module + path.sep + template;
        }
        //file_suffix
...
const migrateCMD = function () {
    console.log();
    console.log('  Migration has been abandoned, please use the database client manually create the table structure.');
    console.log();
    ...
    //     process.exit();
    // }
    // let thinkkoa = require(path.resolve(projectRootPath) + '/node_modules/thinkkoa/lib/thinkkoa.js');
    // if (!thinkkoa) {
    ...
    // let thinkorm = require(path.resolve(projectRootPath) + '/node_modules/thinkorm/index.js');
    // if (!thinkorm) {
    //     console.log('cant find module `thinkorm`, please run `npm install thinkorm`\n');
    //     process.exit();
    // }
    // thinkkoa.loader.loadConfigs(app);
    // let middleware_config = app.configs.middleware.config;
    ...
    //     let dirs = fs.readdirSync(APP_PATH + path.sep + 'model');
...
commander.option('-o, --orm', 'used orm module (thinkorm, liteq), default is liteq, used in `think model` command');
//create project
