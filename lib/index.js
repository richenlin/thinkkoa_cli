 ...
 * @return {} []
 */
const getProjectAppPath = function () {
...
const migrateCMD = function () {
    ...
    var thinkkoa = new (require(path.resolve(projectRootPath) + '/node_modules/thinkkoa/index.js'))({
        root_path: projectRootPath,
        app_path: APP_PATH,
        app_debug: true
    });
    if (!thinkkoa) {
        console.log('thinkorm is not install, please run `npm install thinkkoa`\n');
        process.exit();
    }
    ...
    var middleware_config = think._caches.configs.middleware.config;
...
//create controlelr
program.command('controller <controllerName> <restful>').description('add controller').action(function (controller) {
