...
var createController = function (controller) {
    ...
    mkdir(APP_PATH + path.sep + 'controller' + path.sep + modele);
    if (module) {
        copyFile('controller' + path.sep + 'index.js', APP_PATH + path.sep + 'controller' + path.sep + module + controller + '.js');
    } else {
...
var createModel = function (model) {
    ...
    if (module) {
        copyFile('model' + path.sep + 'model.js', APP_PATH + path.sep + 'model' + path.sep + module + model + '.js');
    } else {
...
var createMiddleware = function (middleware) {
    ...
    if (module) {
        copyFile('middleware' + path.sep + 'middleware.js', APP_PATH + path.sep + 'middleware' + path.sep + module + middleware + '.js');
    } else {
...
var createService = function (service) {
    ...
    if (module) {
        copyFile('service' + path.sep + 'service.js', APP_PATH + path.sep + 'service' + path.sep + module + service + '.js');
    } else {
...
var migrateCMD = function () {
    ...
    var projectRootPath = path.dirname(APP_PATH);
    projectRootPath.indexOf('./') > -1 && (projectRootPath = projectRootPath.replace('./', ''));

    if (!isDir(projectRootPath)) {
        console.log('project app path  `' + projectRootPath + '` is not exist.\n');
        process.exit();
    }
    var thinkorm = require(projectRootPath + '/node_modules/thinkorm/index.js');
    if (!thinkorm) {
        console.log('thinkorm is not install, please run `npm install thinkorm`\n');
        process.exit();
    }

    if (!isFile(projectRootPath + path.sep + 'config' + path.sep + 'config.js')) {
        console.log('project ' + projectRootPath + '/config/middleware.js is not exist.\n');
        process.exit();
    }

    if (!isFile(projectRootPath + path.sep + 'config' + path.sep + 'middleware.js')) {
        console.log('project ' + projectRootPath + '/config/middleware.js is not exist.\n');
        process.exit();
    }

    var middleware_config = thinkorm.require(projectRootPath + path.sep + 'config' + path.sep + 'middleware.js');
    if (middleware_config && middleware_config.config && middleware_config.config.model && middleware_config.config.model.db_type) {
        ...
        var dirs = fs.readdirSync(projectRootPath + path.sep + 'model');
        dirs.forEach(function (dir) {
            if (isDir(dir)) {
                loadFiles({
                    'Model': [
                        projectRootPath + path.sep + 'model' + dir
                    ]
                }, function (t, f, g) {
                    console.log(f);
                ...
                thinkorm.setCollection(thinkorm.require(projectRootPath + path.sep + 'model' + dir), config);
            }
